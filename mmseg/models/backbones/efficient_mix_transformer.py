...
class Selector(nn.Module):
    def __init__(self,C):
        super().__init__()
        self.C = C
        # self.select = nn.Linear(C,C//2)
    def forward(self,x):
        # x: [b,num_head,N,C]
        x_norm = torch.linalg.norm(x,dim=(1,2)).mean(0)
        _,idx = x_norm.topk(self.C//2)
        x = torch.index_select(x, 3, idx)
        return x
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.q_select = Selector(head_dim)
        self.k_select = Selector(head_dim)
